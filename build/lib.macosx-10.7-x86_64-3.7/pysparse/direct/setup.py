...
def getoption(config, section, option):
    try:
        val = config.get(section,option).split(':')
    except:
        val = None
    return val
...
def configuration(parent_package='',top_path=None):
    ...
    suitesparse_libdir = getoption(pysparse_config, 'UMFPACK', 'suitesparse_libdir')
    ...
    if suitesparse_libdir is None or suitesparse_include is None:
        ...
        umfpack_lib.append('suitesparseconfig')
        umfpack_libdir = [suitesparse_libdir]
        ...
        umfpack_include = [suitesparse_include]
